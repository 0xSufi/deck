 ...
 * Ported from Noma Frontend Components to Vanilla JS
 */
...
window.currentBackground = null;

function initBackground(canvasId, activeType) {
    ...
    if (window.currentBackground) {
        window.currentBackground.destroy();
    }

    // Use requested type or random
    const urlParams = new URLSearchParams(window.location.search);
    let type = activeType || urlParams.get('bg');
    ...
    if (type === 'attractor') {
        ...
        // Force type if specific convolution requested
        const convTypes = ["cosmic-quantum", "neural-nexus", "hyperspace", "flow-field", "fourier-spiral"];
        if (convTypes.includes(type)) {
            instance.type = type;
            instance.init(); // Re-init with selected type
        }
    }
    
    window.currentBackground = instance;
}

function switchBackground(type) {
    initBackground('bgCanvas', type);
}